...
// Fixed Python execution endpoint
...
// Enhanced safe Python execution function
...
// Quiz 8: Function Concepts questions route
router.get('/iitm-math-questions/quiz8', async (req, res) => {
  try {
    const { email, count = 50 } = req.query;
    ...
    // Find all available questions excluding completed ones
    let availableQuestions = await IITMathQuestion.find({
      topic: "Function Concepts",  // Topic for Quiz 8
      _id: { $nin: completedQuestionIds }
    });

    console.log(`Found ${availableQuestions.length} new questions available for Function Concepts topic`);

    // Handle case where user has completed most questions
    if (availableQuestions.length === 0) {
      return res.status(200).json({
        message: "All Function Concepts questions completed",
        questions: [],
        resetAvailable: true,
        totalQuestionsInPool: await IITMathQuestion.countDocuments({ topic: "Function Concepts" })
      });
    }
    ...
    console.log(`Returning ${selectedQuestions.length} random Function Concepts questions for user ${email}`);
    ...
    console.error('Error fetching Quiz 8 (Function Concepts) questions:', error);
    res.status(500).json({ error: 'Failed to fetch Function Concepts questions' });
  }
});

// Quiz 9: Logarithmic Functions questions route
