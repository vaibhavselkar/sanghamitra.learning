...
const IITM_CT_Question = require('../model/iitm_ct_questions');
const IITM_CT_Score = require('../model/iitm_ct_scores');
...
// GET CT Questions - Week 1 (Programming Basics)
router.get('/iitm-ct-questions/week1', async (req, res) => {
  try {
    ...
    console.log(`Fetching Week 1 CT questions (Programming Basics) for: ${email}`);
    
    // Find user's completed questions
    let userScore = await IITM_CT_Score.findOne({ email });
    const completedQuestionIds = userScore?.completedQuestionIds || [];
    
    console.log(`User ${email} has completed ${completedQuestionIds.length} CT questions`);

    // Find all available Week 1 questions excluding completed ones
    let availableQuestions = await IITM_CT_Question.find({
      topic: "Programming Basics",  // ✅ CORRECT TOPIC NAME
      _id: { $nin: completedQuestionIds }
    });
    ...
    // Handle case where user has completed all questions
    if (availableQuestions.length === 0) {
      const totalInPool = await IITM_CT_Question.countDocuments({ topic: "Programming Basics" });
      return res.status(200).json({
        message: "All Programming Basics questions completed",
        questions: [],
        resetAvailable: true,
        totalQuestionsInPool: totalInPool
      });
    ...
    console.log(`Returning ${selectedQuestions.length} random CT questions for Programming Basics`);
    ...
    res.status(500).json({ 
      error: 'Failed to fetch Programming Basics questions',
      details: error.message 
    });
  }
});
...
router.post('/iitm-ct-scores', async (req, res) => {
  try {
    const { email, username, quizData } = req.body;
    ...
    // Find existing user or create new one
    let user = await IITM_CT_Score.findOne({ email });
    
    if (!user) {
      // Create new user with completed questions and score
      user = new IITM_CT_Score({ 
        username, 
        email, 
        completedQuestionIds: completedQuestionIds,
        quizScores: [quizData]
      ...
      console.log(`Added ${newCompletedIds.length} new completed CT questions. Total: ${user.completedQuestionIds.length}`);
    }
...
// GET CT Scores
router.get('/iitm-ct-scores', async (req, res) => {
  try {
    const { email, topic } = req.query;

    if (email) {
      const user = await IITM_CT_Score.findOne({ email });
      if (!user) {
        return res.status(404).json({ success: false, message: 'User not found' });
      }
      ...
      const users = await IITM_CT_Score.find({});
      res.json({ success: true, data: users });
    }
  } catch (error) {
...
router.get('/iitm-ct-topics', async (req, res) => {
  try {
    const topics = await IITM_CT_Question.distinct('topic');
    res.status(200).json(topics);
  } catch (error) {
...
router.get('/user-ct-progress/:email', async (req, res) => {
  try {
    ...
    const user = await IITM_CT_Score.findOne({ email });
    const filter = topic ? { topic } : {};
    const totalQuestions = await IITM_CT_Question.countDocuments(filter);
